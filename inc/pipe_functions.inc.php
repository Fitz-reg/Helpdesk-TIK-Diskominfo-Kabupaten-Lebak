...
function hesk_email2ticket($results, $protocol = 0, $set_category = 1, $set_priority = -1)
{
    ...
    $customer_account_required = ! ($hesk_settings['customer_accounts'] === 0 ||
        ($hesk_settings['customer_accounts'] === 1 && $hesk_settings['customer_accounts_required'] === 0));
    ...
    // Pending staff-approval and ambiguous account owners must not be reused.
    if ($tmpvar['customer_id'] === null) {
        // Keep the 3.7.10 notification behavior. Existing templates describe
        // help desks that require an account and are not suitable for ownership conflicts.
        if ($customer_account_required && $hesk_settings['pipe_customer_rejection_notification']) {
            hesk_notifyRejectedCustomer($tmpvar);
        }

        $reason = $customer_account_required
            ? "Could not find customer with email '{$tmpvar['email']}' and helpdesk settings require an account to be created prior to sending tickets via e-mail."
            : "Could not assign customer with email '{$tmpvar['email']}' because its customer account ownership is pending or ambiguous.";
        return hesk_cleanExit($reason);
    }
	...
	if ($is_reply)
	{
        ...
        $ticket['customer_id'] = $tmpvar['customer_id'];
