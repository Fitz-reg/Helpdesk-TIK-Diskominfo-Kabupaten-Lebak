...
if ($customer) {
    ...
    if ($tmpvar['email'] !== '' && hesk_get_customer_account_owning_email($tmpvar['email']) !== null) {
...
if ($hesk_settings['customer_accounts'] && $hesk_settings['customer_accounts_required']) {
    ...
    foreach ($tmpvar['followers'] as $key => $follower) {
        if (($follower_id = hesk_get_or_create_follower($follower)) !== null) {
            $tmpvar['follower_ids'][] = $follower_id;
        } else {
            unset($tmpvar['followers'][$key]);
            $removed_followers[] = $follower;
        }
